...
public class Octree {
	...
	private void checkSize(int vertexCount, float[] data) {
		...
		for(int i = 1; i < vertexCount; i++) {
			if(data[ptr] < p1.x) p1.x = data[ptr];
			else if(data[ptr] > p2.x) p2.x = data[ptr];
			ptr++;
			if(data[ptr] < p1.y) p1.y = data[ptr];
			else if(data[ptr] > p2.y) p2.y = data[ptr];
			ptr++;
			if(data[ptr] < p1.z) p1.z = data[ptr];
			else if(data[ptr] > p2.z) p2.z = data[ptr];
			ptr++;
		}
	}
