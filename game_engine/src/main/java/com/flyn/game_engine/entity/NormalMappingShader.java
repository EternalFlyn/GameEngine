...
public class NormalMappingShader extends ShaderProgram {
	...
	private static final String VERTEX_FILE = "src/main/java/shader/normalMappingShader.vs";
	private static final String FRAGMENT_FILE = "src/main/java/shader/normalMappingShader.fs";
	...
	protected void bindAttributes() {
		...
		bindAttribute(3, "vertexTangents");
	}
	...
	public void setLight(ArrayList<Light> lights, Matrix4f view) {
		for(int i = 0; i < MAX_LIGHTS; i++) {
			if(i < lights.size()) {
				Light light = lights.get(i);
				Vector4f lightPosition = (Vector4f) view.multiply(new Vector4f(light.getPosition(), 1));
				setUniform3f("lightPosition[" + i + "]", new Vector3f(lightPosition.x(), lightPosition.y(), lightPosition.z()));
	...
	public void connectTextureUnit() {
		setUniform1i("textureSampler", 0);
		setUniform1i("normalMapSampler", 1);
	}
